...
1. まず計画を立てる — 何のデータが必要か、どの順番で取るかを自分で判断
2. 複数のツールを自律的に呼び出す — 財務データ、有報テキスト、決算短信、株価を並列取得
3. 途中で検証する — 数字とナラティブに矛盾がないか、データが足りているか
4. レポートを仕上げる — 構造化された分析結果を出力

これを1回の質問で、人間が介在せずにやる。
...
```
Compare Sony and Nintendo's profitability over the last 3 years

What are the risk factors in Nintendo's annual report?

Screen for companies with ROE > 15% and dividend yield > 3%
```
